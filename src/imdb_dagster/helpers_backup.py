...
def get_table_schema(df: pd.DataFrame):
    df = df.reset_index()
    columns = []
    for key in df.keys():
        try:
            columns.append(dg.TableColumn(key, str(df.dtypes[key]), ALL_VALUES[key]))
        except KeyError:
            columns.append(
    ...
    return MetadataValue.table(records=records, schema=dg.TableSchema(columns=columns))
